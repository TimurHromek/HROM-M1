...
class MoELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #   P_i = mean over tokens of gate_probs[:, i] (average router probability for expert i)
        #   f_i = mean over tokens of (expert i is chosen for token) (fraction of tokens routed to expert i)
        #   L_aux = coeff * num_experts * sum_i (f_i * P_i)

        # Create a binary mask indicating which experts were chosen for each token
...
class HROMBlock(nn.Module): # Renamed to HROMBlock_MoE in thought process, but keeping original name for less refactoring
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        # Pre-Normalization with explicit residual connections
        residual1 = x
...
# --- Dataset Loading and Processing --- (Remains Unchanged)
class CombinedChatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(formatted_ids) < 2:
             logging.warning(f"Sequence at index {idx} is too short after processing (<2 tokens). Skipping.")
             return None
...
class HROMTrainer:
    ...
    def train_step(self, batch):
        ...
        with autocast_context:
            ...
            # Model now returns (logits, moe_aux_loss)
        ...
        # Return unscaled main loss and aux loss for logging
...
class SafetyManager:
    def __init__(self, model, tokenizer):
        ...
        self.assistant_id = self.tokenizer.token_to_id("<assistant>")
        self.pad_id = self.tokenizer.token_to_id("<pad>")
        if self.eos_id is None: logging.error("</s> token ID not found!"); self.eos_id = 0
        if self.bos_id is None: logging.error("<s> token ID not found!"); self.bos_id = 0
        if self.user_id is None: logging.error("<user> token ID not found!")
        if self.assistant_id is None: logging.error("<assistant> token ID not found!")
        if self.pad_id is None: logging.error("<pad> token ID not found!"); self.pad_id = 0
    ...
    def content_filter(self, text_ids):
        ...
        for bad_ids in self.bad_word_ids:
            if self.contains_sequence(text_ids, bad_ids):
                ...
                logging.warning(f"Unsafe content detected: Found sequence for '{detected_word}' (IDs: {bad_ids}).")
                return False
        return True
    ...
    def generate_safely(self, prompt, max_new_tokens=50, temperature=0.7, top_k=50):
        ...
        if self.assistant_id is not None:
            ...
            logging.error("Assistant token ID is None for generation.")
        ...
        with torch.no_grad():
            for step in range(max_new_tokens):
                ...
                try:
                    # Model now returns (logits, aux_loss), ignore aux_loss for inference
                    outputs, _ = self.model(current_input_tensor, attention_mask=attention_mask)
...
class CheckpointManager: # (Remains Unchanged in its core logic)
    def __init__(self):
    ...
    def save(self, model, optimizer, step):
        ...
        prefix = os.path.basename(self.checkpoint_dir).replace("checkpoints_", "") # MoE specific prefix
        step_str = str(step)
        filename = f"hrom_moe_{prefix}_step{step_str}_{timestamp}.pt" # MoE in filename
    ...
    def _cleanup_old_checkpoints(self):
        ...
        try:
            ...
            pattern = re.compile(rf"hrom_moe_{prefix}_step(\d+|.+)_(\d{{8}}_\d{{6}})\.pt") # MoE in pattern
            checkpoints = []
        ...
        try:
            ...
            for f in os.listdir(self.checkpoint_dir):
                 ...
                 if match:
                      ...
                      checkpoints.append((filepath, os.path.getmtime(filepath)))
            if not checkpoints:
                logging.info("No valid MoE checkpoints found to load.")
                return 0
            checkpoints.sort(key=lambda x: x[1], reverse=True)
            latest_checkpoint_path, _ = checkpoints[0]
            logging.info(f"Loading latest MoE checkpoint from: {latest_checkpoint_path}")
            ...
            critical_keys = ["dim", "n_layers", "n_heads", "ff_dim", "vocab_size", "max_seq_len",
                             "tokenizer_name", "num_experts", "top_k_experts"] # Added MoE keys
            mismatched_keys = []
            if loaded_config:
                ...
                if mismatched_keys:
                    logging.warning("--- CONFIG MISMATCH DETECTED (MoE Model) ---")
                ...
                logging.warning("Checkpoint does not contain configuration info.")
            try:
                 model.load_state_dict(checkpoint['model'], strict=True) # Strict=True is important
            except RuntimeError as e:
                 logging.error(f"Failed to load model state_dict (MoE): {e}. Check CONFIG or checkpoint. Starting fresh.")
                 return 0
            try:
                 optimizer.load_state_dict(checkpoint['optimizer'])
            except Exception as e:
                 logging.warning(f"Could not load optimizer state_dict (MoE): {e}. Optimizer state reset.")
                 optimizer.state = defaultdict(dict)
            start_step = checkpoint.get('step', 0)
            start_step = max(0, start_step) + 1 if isinstance(start_step, int) else 0
            logging.info(f"MoE Checkpoint loaded. Resuming from optimizer step {start_step}.")
            for state_val in optimizer.state.values():
                for k, v in state_val.items():
                    if isinstance(v, torch.Tensor):
                        try: state_val[k] = v.to(map_location)
                        except Exception as e_opt: logging.error(f"Failed to move optimizer tensor: {e_opt}")
            return start_step
        except FileNotFoundError:
            logging.info(f"No MoE checkpoint directory or files. Starting fresh.")
            return 0
        except Exception as e:
            logging.error(f"Error loading MoE checkpoint: {e}. Starting fresh.")
            return 0
...
def train():
    ...
    if not os.path.exists(tokenizer_path):
        logging.info(f"MoE tokenizer '{CONFIG['tokenizer_name']}' not found. Training...")
        try:
            tokenizer_trainer.train(CONFIG["datasets"])
        except Exception as e:
             logging.error(f"Failed tokenizer training: {e}", exc_info=True)
             return
    else:
        logging.info(f"Loading existing MoE tokenizer from {tokenizer_path}")
    try:
        ...
        logging.info(f"Loaded tokenizer. Vocab: {tokenizer.get_vocab_size()}. Special IDs: PAD={CONFIG['pad_token_id']}, etc.")
    ...
    if CONFIG['vocab_size'] != tokenizer.get_vocab_size():
         logging.warning(f"Config vocab_size ({CONFIG['vocab_size']}) != tokenizer ({tokenizer.get_vocab_size()}). Using tokenizer's.")
         CONFIG['vocab_size'] = tokenizer.get_vocab_size()
    model = HROM() # This now creates the MoE version due to HROMBlock changes
    ...
    try:
         logging.info("Pre-loading/caching datasets for MoE training...")
         ...
         if len(dataset) == 0:
             logging.error("Dataset is empty after processing. Cannot train MoE model.")
             return
         dataloader = DataLoader(
             dataset, batch_size=CONFIG["batch_size"], collate_fn=CombinedChatDataset.collate_fn,
             shuffle=True, num_workers=min(4, os.cpu_count() // 2 if (os.cpu_count() and os.cpu_count() > 1) else 1),
             pin_memory=torch.cuda.is_available(),
             prefetch_factor=2 if torch.cuda.is_available() and os.cpu_count() and os.cpu_count() > 1 else None,
             drop_last=False
         )
    except Exception as e:
         logging.error(f"Failed to initialize dataset/dataloader for MoE: {e}", exc_info=True)
         return

    logging.info("Initializing Trainer, Checkpoint Manager, and Safety Manager for MoE...")
    ...
    logging.info(f"Starting MoE training from optimizer step {start_optimizer_step}")
    optimizer_step = start_optimizer_step
    
    # For logging accumulated losses over grad_accum_steps
    accum_main_loss_for_log = 0.0
    accum_aux_loss_for_log = 0.0
    
    batch_step = optimizer_step * CONFIG["grad_accum_steps"]
    start_epoch = batch_step // len(dataloader) if len(dataloader) > 0 else 0

    try:
        if len(dataloader) == 0: raise ValueError("DataLoader has zero length.")
        total_optimizer_steps_estimate = (len(dataloader) * CONFIG["num_epochs"]) // CONFIG["grad_accum_steps"]
        logging.info(f"Estimated dataset size: {len(dataset)}, Batches/epoch: {len(dataloader)}")
        logging.info(f"Grad Accum: {CONFIG['grad_accum_steps']}, Effective Batch: {CONFIG['batch_size'] * CONFIG['grad_accum_steps']}")
        logging.info(f"Target Epochs: {CONFIG['num_epochs']}, Est. Total Opt Steps: {total_optimizer_steps_estimate}")
    except Exception as e:
        logging.warning(f"Could not estimate total steps: {e}")

    model.train()
    for epoch in range(start_epoch, CONFIG["num_epochs"]):
        logging.info(f"--- Starting MoE Epoch {epoch+1}/{CONFIG['num_epochs']} ---")
        epoch_main_loss = 0.0
        epoch_aux_loss = 0.0
        ...
        for i, batch in enumerate(dataloader):
            ...
            main_loss_val, aux_loss_val = trainer_obj.train_step(batch) # Returns (main_loss, aux_loss)

            # Check for NaN/Inf in losses
            valid_loss = True
            if main_loss_val is None or torch.isnan(torch.tensor(main_loss_val)) or torch.isinf(torch.tensor(main_loss_val)):
                 logging.error(f"NaN/Inf main loss: {main_loss_val}. Aux: {aux_loss_val}. Opt Step {optimizer_step}. Stopping.")
                 checkpoint_manager.save(model, trainer_obj.optimizer, f"{optimizer_step}_main_loss_error")
                 valid_loss = False
            if aux_loss_val is None or torch.isnan(torch.tensor(aux_loss_val)) or torch.isinf(torch.tensor(aux_loss_val)):
                 # Don't stop for aux loss NaN/Inf immediately, but log it. Could make it 0.
                 logging.warning(f"NaN/Inf aux loss: {aux_loss_val}. Main: {main_loss_val}. Opt Step {optimizer_step}. Problematic.")
                 # If aux_loss is NaN, total_loss will be NaN, and scaled_loss.backward() will propagate NaNs.
                 # Consider setting aux_loss_val = 0.0 here if it's problematic. For now, let it propagate.
                 # If main_loss was also bad, we're already stopping.
                 if valid_loss == False: return # Already stopping from main_loss error
                 # If only aux_loss is NaN, it will make total_loss NaN and scaled_loss.backward() will likely fail or result in NaN grads.
                 # This should ideally be caught by gradient scaler or clip_grad_norm if grads become NaN/Inf.

            if not valid_loss: return # Stop if main loss was bad

            accum_main_loss_for_log += main_loss_val
            accum_aux_loss_for_log += aux_loss_val
            epoch_main_loss += main_loss_val
            ...
            if batch_step % CONFIG["grad_accum_steps"] == 0:
                ...
                avg_main_loss = accum_main_loss_for_log / CONFIG["grad_accum_steps"]
                avg_aux_loss = accum_aux_loss_for_log / CONFIG["grad_accum_steps"]
                accum_main_loss_for_log = 0.0
                accum_aux_loss_for_log = 0.0

                if optimizer_step % CONFIG["debug_interval"] == 0:
                    logging.info(
                        f"E {epoch+1} | OptSt {optimizer_step} | BtSt {batch_step} | "
                ...
                if optimizer_step > 0 and optimizer_step % CONFIG["checkpoint_interval"] == 0:
                    logging.info(f"Checkpoint interval reached at MoE optimizer step {optimizer_step}.")
                    checkpoint_manager.save(model, trainer_obj.optimizer, optimizer_step)
                    safety.debug_generation("<user> Hi! How are you?")
                
                optimizer_step += 1

        avg_epoch_main_loss = epoch_main_loss / num_batches_in_epoch if num_batches_in_epoch > 0 else 0
        avg_epoch_aux_loss = epoch_aux_loss / num_batches_in_epoch if num_batches_in_epoch > 0 else 0
        logging.info(
            f"--- Finished MoE Epoch {epoch+1}/{CONFIG['num_epochs']} | "
    ...
    logging.info(f"MoE Training finished after {CONFIG['num_epochs']} target epochs.")
    logging.info("Saving final MoE model state...")
